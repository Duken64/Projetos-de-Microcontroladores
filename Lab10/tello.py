...
class Tello:
    ...
    def __init__(self, ssid, test_mode=False):
        ...
        #self.TELLO_SSID = "TELLO-C7AC08" #Tello Micro
        #self.TELLO_SSID = "TELLO-AC31C6" #Tello Henrique
        conectado = True
...
#            conectado = self._conectar_tello_win()
    ...
    def get_video_address(self):
        return "udp://@%s:%s"%(self.ip_local_image,self.porta_local_image)
    
    def get_video_cap(self):
        if self.cap is None:
            self.cap = cv.VideoCapture(self.get_video_address)
